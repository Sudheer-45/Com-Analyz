...
// Removed: import curatedQuestionsRoutes from './routes/curatedQuestions.routes.js'; // This was for datasets
...
app.post('/api/ai/jd-chat/start', async (req, res) => {
    ...
    try {
        ...
        console.error("Error starting JD chat:", error);
...
app.post('/api/ai/jd-chat/message', async (req, res) => {
    // Check if AI chat is configured before proceeding
    if (!(await checkChatAiConfiguration())) {
        return res.status(503).json({ error: "AI chat service is not configured correctly. Check API key or model availability." });
    }
    ...
    try {
        ...
        console.error("Error sending JD chat message:", error);
        // Check for specific error types from Google Generative AI
        if (error.message && error.message.includes('400') && error.message.includes('Content policy enforcement')) {
...
// Removed: app.use('/api/curated-questions', curatedQuestionsRoutes); // This was for datasets
